...
data <- read.csv("F:/Thesis/ColonCancer/UpDown.csv", row.names=1)
...
for (i in 1:nrow(data)){
  vec1 = as.numeric(data[i:i,1:22])
  vec2 = as.numeric(data[i:i,23:62])
...
legend(1,4.50,legend=c("Group1","Group2"),fill=c('#f92c32','#127cd4'),cex=0.7)

write.csv(resUp, file="F:/Thesis/ColonCancer/upregulated.csv")
write.csv(resDown, file="F:/Thesis/ColonCancer/downregulated.csv")
